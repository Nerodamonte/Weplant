...
public class SupabaseService {
    ...
    public void deleteFile(String path) throws Exception {
        // Gửi DELETE lên Supabase Storage API
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(props.getUrl() + "/storage/v1/object/" + props.getBucket() + "/" + path))
                .header("Authorization", "Bearer " + props.getServiceRoleKey())
                .DELETE()
                .build();
        HttpResponse<String> response = http.send(request, HttpResponse.BodyHandlers.ofString());
        if (response.statusCode() >= 300) {
            throw new IOException("Supabase delete failed: " + response.body());
        }
    }
}
